...
if selected_ml_algorithm == 'classification':
    ...
    with st.container():
        ...
        with col1:
            ...
            df = pd.DataFrame({'Estimator': estimators, 'aeid': aeids, 'Precision': precision, 'Recall': recall})
            ...
            fig = px.scatter(df, x='Recall', y='Precision', color='Estimator', title='Precision vs Recall by Estimator', hover_data=['aeid'], opacity=0.6, **trendline_args, **hisogram_args)
        ...
        with col2:
            i = 1
            threshold = threshold_names[i]
            # Plot precision vs recall for all aeids and all estimators (the estimators are color-encoded)
            # fig = go.Figure()
            # for estimator_model in reports.keys():
            #     for aeid in reports[estimator_model].keys():
            #         fig.add_trace(go.Scatter(x=reports[estimator_model][aeid][threshold]['recall'],
            #                                 y=reports[estimator_model][aeid][threshold]['precision'],
            #                                 mode='markers',
