...
from ml.src.pipeline.ml_helper import load_config
from ml.src.pipeline.constants import METADATA_DIR_PATH, FILE_FORMAT

CONFIG = load_config(only_load=1)

src_path = os.path.join(METADATA_DIR_PATH, 'fps', f"{CONFIG['fingerprint_file']}.csv")
dest_path = os.path.join(METADATA_DIR_PATH, 'fps', f"{CONFIG['fingerprint_file']}{FILE_FORMAT}")
