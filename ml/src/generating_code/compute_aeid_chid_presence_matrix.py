...
from ml.src.constants import METADATA_DIR_PATH, METADATA_SUBSET_DIR_PATH
...
if ALL:
    ...
    aeid_compound_mapping_path = os.path.join(METADATA_DIR_PATH, f"aeid_compound_mapping{config['file_format']}")
    ...
    aeid_compound_presence_matrix_path = os.path.join(METADATA_DIR_PATH, f"aeid_compound_presence_matrix{config['file_format']}")
    presence_matrix.to_parquet(aeid_compound_presence_matrix_path, compression='gzip')

    # Get compounds
    compounds = presence_matrix.columns.tolist()
    compounds_path = os.path.join(METADATA_DIR_PATH, f"compounds_tested{config['file_format']}")
...
if SUBSET:
    ...
    aeid_compound_presence_matrix_path = os.path.join(METADATA_SUBSET_DIR_PATH,
                                                  f"aeid_compound_presence_matrix{config['file_format']}")
    presence_matrix.to_parquet(aeid_compound_presence_matrix_path, compression='gzip')
